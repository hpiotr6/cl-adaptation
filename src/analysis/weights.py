...
def init_wandb(data_task, cfg, task_idx, exp_name):
    ...
    project_name = f"2205new_weights-{today_date}"
...
def my_app(config: DictConfig) -> None:
    ...
    for data_task in range(5):
        ...
        for task_idx, ckpt in enumerate(sorted(model_factory.ckpts)):
            ...
            linear_probing_model = LinearProbingModel(
                model.model, num_classes, data_task
            )
...
# MAX_EPOCHS = 30
# root = Path("new_linear_checkpoints")
...
#         model = model_factory.create_model(task=task_idx, num_classes=num_classes)
#         linear_probing_model = LinearProbingModel(
#             model.model, num_classes, task_idx
#         )
