...
class GAN(pl.LightningModule):
        ...
        **kwargs
        ...
        if generator_type == "linear":
            self.generator = LinearGenerator(
                input_dim + conditional_dim, output_dim
            ).to(device)
        elif generator_type == "veegan":
            self.generator = DCGanGenerator(latent_dim=input_dim + conditional_dim).to(
                device
            )
    ...
    def generate_img_grid(self, num_samples=25):
        conditional = self.random_conditional(num_samples, self.conditional_dim)
    ...
    def configure_optimizers(self):
        opt_g = torch.optim.Adam(
            itertools.chain(
                self.generator.parameters(), self.mi_estimator.parameters()
            ),
            lr=self.lr,
            betas=(0.5, 0.999),
        )
