...
class Mine(nn.Module):
    ...
    def optimize(self, X, Y, iters, batch_size, opt=None):
        ...
        for iter in range(1, iters + 1):
            mu_mi = 0
            for x, y in helpers.batch(X, Y, batch_size):
                opt.zero_grad()
...
class MutualInformationEstimator(pl.LightningModule):
    ...
    def test_end(self, outputs):
        avg_mi = (
            torch.stack([x["test_mi"] for x in outputs]).mean().detach().cpu().numpy()
        )
    ...
    def train_dataloader(self):
        ...
        train_loader = torch.utils.data.DataLoader(
            FunctionDataset(
                self.kwargs["N"], self.x_dim, self.kwargs["sigma"], self.kwargs["f"]
            ),
...
def function_experiment():
    ...
    for sigma in sigmas:
        for ix, f in enumerate(fs):
            ...
            model = MutualInformationEstimator(dim, dim, loss="mine", **kwargs).to(
                device
            )
...
def rho_experiment():
    ...
    for rho in rhos:
        ...
        model = MutualInformationEstimator(dim, dim, loss="mine_biased", **kwargs).to(
            device
        )
        trainer = Trainer(max_epochs=epochs, early_stop_callback=False, gpus=1)
        trainer.fit(model)
